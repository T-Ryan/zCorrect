...
//	string path2="C:Data:xCorrect:VIP_Boutons:2018.12.04 Herminio"
	...
	FOR(FolderNum=0;FolderNum<1;FolderNum+=1)
		...
		for(i=0;i<(Length/2);i+=1)
			...
			print message
			AutoLoadScanImageSingle(pathstr, RecFileName)
			...
			killwaves $waveN    		     		    	      // It should split the channels in n channels and name it automatically  'blabla_ch1'and 'blabla_ch2'
		
			print "Loading zStack for this experiment"
			AutoLoadScanImageSingle(pathstr, zstackFileName)
			...
			//outputs: VolumeCh1, VolumeCh2, shiftXReg, shiftYReg
			string Ch1Ref="VolumeCh1_PlReg"
			string Ch2Ref="VolumeCh2_PlReg"
			string VolCh1SD="VolumeStDevCh1_Plreg" // N.B.  remove the "_PlReg" if using CreateRefStack_3DReg
			...
			wave M_stdvImage, W_coef, W_sigma, Zestimate
...
function RegisterAll(ImPlane,Ch1MovieName, Ch2MovieName, RefStack_Ch1, RefStack_Ch2, VolCh1SD,Ch1PosSDMovieName, Ch1NegSDMovieName)
	...
	// Currently registers to the first frame, uncomment to average first 5 frames of the movie to use as a reference 			
...
//Check for offsets // DONE TR 27/01
// TR- modified to generate a correlation map over time. 01/06/19

//speed per frame - 28/01/19 - after optimizing
// - Sample		Downsamplexy	Dimensions		Laptop/Comp		Result 
// - Beads		1					256x100		 	Laptop			0.82s
// - Vessel		2					512x100			Laptop			0.69s

// -EDIT TR 12/03/19 - added option to downsample in x and y dimensions seperately (images are typically 512 x 100)
...
function Z_estimate(MovieStackIn, RefStackIn)
	...
	//tic() //for timing while getting code right and fast
...
make/o/n=(zdim,nFrames) CorrMap
	...
	for(j=0; j<nFrames; j+=1)
		...
		for(i=0; i<zdim; i+=1)
			duplicate/FREE/o/r=[][][i] RefStack_meansub RefFrame // edit TR - 28/01/19
			redimension/n=(-1,-1) RefFrame
...
// z-stacks are split into their individual planes and realigned to the central (chronological) image.
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
// Thomas Ryan 14/05/19 Script to complete last stage of analysis on series of experiments with existing QA waves //
/////////////////////////////////////////////////////////////////////////////////////////////////////////

function postProcess()

	
	string objList="CroppedCh1_SD_QA_Nor;CroppedRef_Ch1_reg_SD_QA;zestimate"

	string path1="C:\Data\xCorrect\VIP_Boutons\ToPostProcess"
	//string path2=				// can add more folders here if desired, it will loop through them sequentially
	
	string S_path
	variable FolderNum,k
	...
	FOR(FolderNum=0;FolderNum<1;FolderNum+=1)
		...
		newpath/O pathstr, S_path			
		string filelist= indexedfile(pathstr,-1,".pxp")	
		filelist=SortList(filelist, ";", 16)				  	  	//Create a list of all the .pxp files in the folder. -1 parameter addresses all files.
		
		Variable Length=itemsinlist(filelist)	// count how many files are in this dir
		string pathstr=s_path
		print pathstr
		...
		for(i=0;i<Length;i+=1)	// cycle through files in the folder
			...
			// Now we want to search for the stimulus and locomotion channels... the string WName specifies a prefix you want to search the experiment for. 
			// The script will then load all strings matching that prefix and ending with a single number
		
		//	if(k==1)
				string WName="RecStimB_0000"
			//else 
		//		WName="RecStimA_0000"
		//	endif
			
			LoadMatchesCh3 (WName, expPath)
			LoadMatchesCh4 (WName, expPath)
			///////// something going wrong in the batch... just save the files with the waves we need for now
			...
			//LocTrace(locoN)
			duplicate/o $zest zEstimateFlipped
			zEstimateFlipped*=-1
			string expN=RemoveEnding(expName, ".pxp")
			
			string saveName = expN + "_Corr.pxp"
			string recList = waveList("Rec*", ";","")
			wave VoltTrace,VTrace
			killwaves VoltTrace,VTrace
			killAllWavesList(recList)
		
			message = "Saving experiment as " + pathstr + "\\" + saveName
			print message
			saveExperiment/P=pathstr as saveName
			killallwaves()
		endfor
...
// Now we have all the things from this experiment we need we can run the correction and correlation scripts on it
		
			TR_CorrectAndCorrelate()
		
			// Delete the channel 3 and 4 movies as they are big and no sense duplicating them
...
function PoolData()
	...
	string pathstr="C:\Data\xCorrect\VIP_Boutons\Analyzed_Stim\Correlation_Analysis",subDataFolder="Correlation_CorrectedAll",objList="List_Corr_LOCO_CCValues",targetStr="CorrectedAllCCValues_Concat"	
	...
	for(i=0;i<numFiles;i+=1)	// cycle through files in the folder
		...
		string expName= StringFromList(i, filelist)
		string message = "Loading from Experiment " + expName + " ( " + num2str(i) + "/" +num2str(numFiles) + " in this folder"
		print message
...
string savename="C:\Data\xCorrect\VIP_Boutons\ToPostProcess_Anacleto2\190122GumerA8.pxp"
string expPath="C:\Data\xCorrect\VIP_Boutons\postprocessedtemp\190122GumerA8.pxp"
string objlist="zestimate;CroppedCh1_SD_QA;CroppedRef_Ch1_reg_SD_QA"




string Ch3="RecStimA_00008_Ch3"
string Ch4="RecStimA_00008_Ch4"

objlist = AddListItem(Ch3,objList,";")
objlist = AddListItem(Ch4,objList,";")
...
string expPath="C:\Data\xCorrect\VIP_Boutons\postprocessedtemp\181212GumerA4.pxp"
string objlist="CroppedCh1;CroppedRef_Ch1_reg"
